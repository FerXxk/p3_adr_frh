...
class ExtendedKalmanFilter:
	...
	def predict(self, u, dt):
		...
		self.Ga = self.G(self.mu, u, dt)

        # 3. Update the covariance
		self.Sigma = self.Ga @ self.Sigma @ self.Ga.T + self.R
	...
	def update(self, z, dt):
		...
		self.Ha = self.H(self.mu)

        # 2. Innovation covariance
    
		S = self.Ha @ self.Sigma @ self.Ha.T + self.Q

        # 3. Kalman gain
		K = self.Sigma @ self.Ha.T @ np.linalg.inv(S)
		...
		self.Sigma = (I - K @ self.Ha) @ self.Sigma
